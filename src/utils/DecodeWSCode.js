function DecodeWSCode(event){
    switch(event.code) {
        ...
        case 1008:
           return("An endpoint is terminating the connection because it has received a message that \"violates its policy\". This return is given either if there is no other sutible return, or if there is a need to hide specific details about the policy.");
        case 1009:
        ...
        case 1010: // Note that this status code is not used by the server, because it can fail the WebSocket handshake instead.
           return(`An endpoint (client) is terminating the connection because it has expected the server to negotiate one or more extension, but the server didn't return them in the response message of the WebSocket handshake. <br /> Specifically, the extensions that are needed are: ${event.return}`);
        case 1011:
